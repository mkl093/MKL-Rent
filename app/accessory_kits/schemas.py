"""Pydantic-схемы комплекта аксессуаров."""

from __future__ import annotations

from decimal import Decimal

from pydantic import BaseModel, Field

from app.inventory.enums import KitWeightMode


class AccessoryKitInput(BaseModel):
    name: str = Field(min_length=1, max_length=255)
    barcode: str | None = Field(default=None, max_length=128)
    weight_mode: KitWeightMode = KitWeightMode.CONTENT
    weight_value: Decimal | None = Field(default=None, ge=0)
    length_mm: int = Field(default=0, ge=0)
    width_mm: int = Field(default=0, ge=0)
    height_mm: int = Field(default=0, ge=0)
    comment: str | None = None


class CustomAccessoryKitLine(BaseModel):
    name: str = Field(min_length=1, max_length=255)
    quantity: int = Field(default=1, ge=1)
    unit_weight_kg: Decimal = Field(default=Decimal("0"), ge=0)
    comment: str | None = None


class AccessoryKitTemplateInput(BaseModel):
    name: str = Field(min_length=1, max_length=255)
    comment: str | None = None


class TemplateCustomLine(BaseModel):
    name: str = Field(min_length=1, max_length=255)
    quantity: int = Field(default=1, ge=1)
    comment: str | None = None
